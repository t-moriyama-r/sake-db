...
- **変更内容**: MongoDBの`liquors`コレクションの`name`フィールドに通常のB-treeインデックスを使用
- **目的**: 正規表現検索のパフォーマンス向上
...
- **機能**:
  - MongoDBの正規表現検索（`$regex`クエリ）を使用
  - 部分一致検索（大文字小文字を区別しない）
  - 複数キーワード対応: スペース（半角・全角）区切りでOR検索
  - 結果数の制限
  - 例: 「ス」→「スペイサイド」「スコッチ」等がヒット
  - 例: 「スペイ サイド」→「スペイサイド」「サイダー」等がヒット（OR検索）
...
# 複数キーワードでのOR検索例
query {
  searchLiquors(keyword: "スペイ サイド", limit: 50) {
    id
    name
    categoryName
  }
...
### 検索速度（予測値）
- 小規模（～10,000件）: 10-30ms
- 中規模（10,000～100,000件）: 30-100ms
- 大規模（100,000～1,000,000件）: 100-300ms
- 超大規模（1,000,000件～）: 300-800ms

### 制限事項
- 最大検索結果数: 1000件（バックエンド側で制限）
- フロントエンドデフォルト表示数: 100件
- 正規表現による部分一致検索
- 複数キーワードはOR検索（スペース区切り）
